...
RSpec.describe UserFilters::RemainingCars do
  describe '#call' do
    subject(:context) { described_class.call(scope: Car.joins(:brand), user: user, collection_ids: collection_ids, external_cars_recommended: {}) }
    ...
    context 'without filters' do
      ...
      it 'returns the collection ids' do
        expect(context.collection_ids).to eq([perfect_match_car.id, preferred_brand_car.id, external_car.id, remaining_car.id])
      end
    end
  end
end
